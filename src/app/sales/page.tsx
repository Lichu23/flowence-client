...
function SalesContent() {
  ...
  const [status, setStatus] = useState<'completed' | 'refunded' | 'cancelled' | 'pending' | ''>('');
  ...
  const handleSearch = async () => {
    ...
    setLoading(true);
    try {
      const result = await salesApi.searchByTicket(currentStore!.id, searchTerm.trim());
  ...
  const handleBarcodeScan = async (barcode: string) => {
    ...
    setLoading(true);
    try {
      const result = await salesApi.searchByTicket(currentStore!.id, barcode);
      router.push(`/sales/${result.sale.id}`);
    } catch (e) {
      console.error(e);
      toast.error('Venta no encontrada con el código escaneado');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="text-center">
          <p className="text-sm sm:text-base text-gray-600">Selecciona una tienda para ver las ventas</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
          <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">Ventas</h2>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
          <div className="bg-white p-3 sm:p-4 rounded-lg shadow-sm border border-gray-200">
            ...
            {(method || status) && (
              <p className="text-xs text-blue-600 mt-1">({stats.totalSales} en página)</p>
            )}
          </div>
          <div className="bg-white p-3 sm:p-4 rounded-lg shadow-sm border border-gray-200">
            <p className="text-xs text-gray-600 mb-1">Ingresos Totales</p>
            <p className="text-xl sm:text-2xl font-bold text-green-600">{formatCurrency(stats.totalRevenue)}</p>
          </div>
          <div className="bg-white p-3 sm:p-4 rounded-lg shadow-sm border border-gray-200">
            <p className="text-xs text-gray-600 mb-1">Efectivo</p>
            <p className="text-xl sm:text-2xl font-bold text-gray-900">{stats.cashSales}</p>
          </div>
          <div className="bg-white p-3 sm:p-4 rounded-lg shadow-sm border border-gray-200">
            <p className="text-xs text-gray-600 mb-1">Tarjeta</p>
            <p className="text-xl sm:text-2xl font-bold text-gray-900">{stats.cardSales}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-3 sm:p-4 mb-4 sm:mb-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            ...
            <div className="sm:col-span-2">
              <label className="block text-xs font-medium text-gray-700 mb-1">Buscar por Ticket/Código</label>
            ...
            <div>
              ...
              <select 
                value={status} 
                onChange={e => {
                  setStatus(e.target.value as 'completed' | 'refunded' | 'cancelled' | 'pending' | '');
                  setPage(1);
                }}
                className="w-full px-3 sm:px-4 py-2 sm:py-2.5 text-sm sm:text-base border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          {loading ? (
            <div className="p-6 sm:p-8 text-center">
              ...
              <p className="text-sm sm:text-base text-gray-600 mt-4">Cargando ventas...</p>
            </div>
          ) : sales.length === 0 ? (
            <div className="p-6 sm:p-8 text-center">
              ...
              <p className="text-sm sm:text-base text-gray-600">
                {method || status ? 'No se encontraron ventas con los filtros aplicados' : 'No hay ventas registradas aún'}
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="hidden lg:block overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {sales.map((s) => (
                      <tr key={s.id}>
                        <td className="px-4 py-3 text-sm text-gray-900">{formatDateTime(new Date(s.created_at))}</td>
                        <td className="px-4 py-3">
                          <div className="text-sm font-medium text-gray-900">{s.receipt_number}</div>
                        </td>
                        <td className="px-4 py-3">
                          <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                            ...
                            {s.payment_method === 'cash' ? 'Efectivo' : s.payment_method === 'card' ? 'Tarjeta' : 'Mixto'}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                            s.payment_status === 'completed' ? 'bg-green-100 text-green-800' :
                            s.payment_status === 'refunded' ? 'bg-orange-100 text-orange-800' :
                            s.payment_status === 'cancelled' ? 'bg-red-100 text-red-800' :
                            'bg-yellow-100 text-yellow-800'
                          }`}>
                            {s.payment_status === 'completed' ? 'Completado' :
                             s.payment_status === 'refunded' ? 'Reintegrado' :
                             s.payment_status === 'cancelled' ? 'Cancelado' : 'Pendiente'}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-right">
                          <div className="text-sm font-medium text-gray-900">{formatCurrency(Number(s.total))}</div>
                        </td>
                        <td className="px-4 py-3 text-right">
                          <div className="flex gap-2 justify-end">
                            <button
                              onClick={async () => {
                                try {
                                  await salesApi.downloadReceipt(currentStore.id, s.id);
                  ...
                  <div key={s.id} className="p-4">
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex-1 min-w-0 mr-2">
                        <h3 className="text-base font-semibold text-gray-900">{s.receipt_number}</h3>
                        <p className="text-xs text-gray-500 mt-0.5">{formatDateTime(new Date(s.created_at))}</p>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-3">
                      <div className="bg-gray-50 p-2 rounded">
                        ...
                        <p className="text-base font-bold text-gray-900">{formatCurrency(Number(s.total))}</p>
                      </div>
                      <div className="bg-gray-50 p-2 rounded">
